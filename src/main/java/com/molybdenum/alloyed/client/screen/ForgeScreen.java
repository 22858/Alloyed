...
public class ForgeScreen extends AbstractContainerScreen<ForgeMenu> {
	private static final ResourceLocation TEXTURE = Alloyed.asResource("textures/gui/forge_gui.png");
	...
	protected void renderBg(GuiGraphics guiGraphics, float partialTick, int mouseX, int mouseY) {
		...
		if (menu.isCrafting()) {
			guiGraphics.blit(TEXTURE, x + 89, y + 17, 176, 14, menu.getScaledProgress(), 17, 256, 256);
		}

		if (menu.isFueled()) {
			float currentHeight = menu.getLitTime();
			int offset = (int) (15- currentHeight);
			guiGraphics.blit(TEXTURE, x + 94, y + 36+offset, 176, offset, 14, (int) currentHeight, 256, 256);
		}
	}
