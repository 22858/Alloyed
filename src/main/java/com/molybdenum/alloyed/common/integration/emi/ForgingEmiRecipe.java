...
public class ForgingEmiRecipe implements EmiRecipe {
    ...
    public ForgingEmiRecipe(ResourceLocation id, AbstractForgingRecipe recipe) {
        this.id = id;
        this.input = padIngredients(recipe);
        this.output = EmiStack.of(recipe.getResultItem(RegistryAccess.EMPTY));
        this.cookTime = recipe.getCookTime();
        this.shapeless = !(recipe instanceof ShapedForgingRecipe);
    }

    private static List<EmiIngredient> padIngredients(AbstractForgingRecipe recipe) {
        if (recipe instanceof ShapedForgingRecipe shapedRecipe) {
    ...
    public EmiRecipeCategory getCategory() {
        return AlloyedEMIPlugin.FORGING;
    }
